    ...
    def update_metrics(self, preds, batch):
        ...
        for si, pred in enumerate(preds):
            ...
            if self.args.save_txt:
                ...
                self.save_one_txt(predn, self.args.save_conf,
                                  pbatch["ori_shape"], file)
    ...
    def print_results(self):
        ...
        if self.args.verbose and not self.training and self.nc > 1 and len(self.stats):
            for i, c in enumerate(self.metrics.ap_class_index):
                LOGGER.info(pf % (
                    self.names[c], self.seen, self.nt_per_class[c], *self.metrics.class_result(i)))
    ...
    def eval_json(self, stats):
        ...
        if self.args.save_json and self.is_coco and len(self.jdict):
            anno_json = self.data["path"] / \
                "annotations/instances_val2017.json"  # annotations
